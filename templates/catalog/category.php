      ...
      <?php foreach ($term_parent_children as $key => $term_parent_child) : ?>
        ...
        <?php if ( $sub_term_parent_children && !empty($sub_term_parent_children) && is_array($sub_term_parent_children) && !is_wp_error($sub_term_parent_children) ) : ?>
          <li class="filter__wrap filter__wrap--open">
            ...
            <ul class="filter__list"> 
              <?php foreach ($sub_term_parent_children as $key => $sub_term_parent_child) : ?>
                <?php 
                  $sub_term_parent_child_link = get_term_link( $sub_term_parent_child, $taxonomy_aside ) ?? '';  
                  $sub_term_parent_child_mame = $sub_term_parent_child->name ?? ''; 
                ?>
                <li class="filter__item">
                  <a class="filter__link <?= ($sub_term_parent_child === $term) ? 'filter__link--active' : ''; ?>" href="<?= $sub_term_parent_child_link; ?>">
                    <?= $sub_term_parent_child_mame; ?>
                  </a>
                </li> 
              <?php endforeach; ?>   
